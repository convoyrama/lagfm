...
export default function VolumeDial({ volume, setVolume, onAir, togglePlay, showVolume, setShowVolume }) {
  ...
  return (
    <div className="dial-wrapper" ref={dialRef} 
      ...
      <svg className="volume-ring" viewBox="0 0 200 200">
        <circle cx="100" cy="100" r="90" className="ring-bg" />
        <circle cx="100" cy="100" r="90" className="ring-fill" 
                style={{ strokeDashoffset: 565 - (565 * volume) }} />
      </svg>
      ...
      >
        {onAir ? <Pause size={64} fill="currentColor" /> : <Play size={64} fill="currentColor" />}
      </button>
