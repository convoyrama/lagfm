...
export default function LAGNodeHub() {
  ...
  // Estados para el limitador (0: off, 1: visual, 2: visual+audio)
  ...
  const defaultTelemetry = {
    ...
    truck: { 
      speed: 0, rpm: 0, gear: 'N', fuel: 0, fuelCapacity: 0, avgConsumption: 0, damage: 0,
  ...
  // Fuel: Litros o Galones
  const displayFuel = units === 'KM' ? Math.round(data.truck?.fuel || 0) : ((data.truck?.fuel || 0) * 0.264172).toFixed(1);
  ...
  return (
    <div className={`hub-panel ${!isConnected ? 'offline' : (data.game?.gameName?.includes("Demo") ? 'demo-mode' : '')}`}>
      ...
      <div className={`hub-core-display ${isConnected ? 'active' : 'idle'}`}>
        ...
        <div className="hub-main-container">
          ...
          <div className="hub-main">
            ...
            <div className="data-row">
              <div className={`fuel-numeric ${(data.truck?.fuel || 0) <= 50 ? 'critical-fuel' : ''}`}>
                <Fuel size={12} className="label-icon" />
                <span className="value">{displayFuel}</span>
                <span className="unit">{units === 'KM' ? 'L' : 'GAL'}</span>
              </div>
        ...
        {/* 6. FOOTER STATUS (Daño, Unidades, Remolque) */}
